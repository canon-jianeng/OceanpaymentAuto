...
class TestGateway(unittest.TestCase):
    ...
    @ddt.data(*Exc(Gateway().read_data("auth"), "Auth").read_merged())
    def test_auth(self, test_data):
        ...
        # 断言：对账状态
        res_dict = test_data["CheckBefore"]
        for key_val in res_dict:
            self.assertEqual(
                res_dict[key_val],
                Res.Result(self.driver).back_check(key_val), "预授权之前, 对账状态错误"
            )
        # 进入预授权
        ...
        Che.Action(self.driver).put_check(test_data["AutoCheck"], num)
        # 断言：对账状态
        res_dict = test_data["CheckAfter"]
        for key_val in res_dict:
            self.assertEqual(
                res_dict[key_val],
                Res.Result(self.driver).back_check(key_val), "预授权之后, 对账状态错误"
            )
