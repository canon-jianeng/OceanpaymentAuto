...
class Action(object):
    ...
    def put_create(self, test_data=None):
        ...
        addr = self.gate.read_val("gateway", "proj_domain") + self.gate.read_val("addr", "twhcre")
        ...
        test_pay.click_submit("make payment")
    ...
    def put_pay(self, test_data, quickid):
        ...
        test_pay = Sub.SubmitPage(self.driver)
        log.info("----- 页面标题: %s -----" % self.driver.title)
        addr = self.gate.read_val("gateway", "proj_domain") + self.gate.read_val("addr", "twhpay")
